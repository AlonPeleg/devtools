...
**"Open with Dev Toolkit"** — once installed in Chrome or Edge, the app registers itself for code files, `.json` / `.xml` and `.csv` / `.tsv` / `.xlsx` / `.xls`. Right-click a file on your desktop and choose **Open with → Dev Toolkit**, double-click a file whose default app is set to Dev Toolkit, or drop one onto the app's icon or taskbar/dock entry. Dev Toolkit opens (or comes to the front) on the tab that matches the file — JSON/XML entries go to the JSON tab, spreadsheet files go to CSV/XLSX, anything else goes to Code — and adds it there, exactly like using that tab's own **Add / Upload**. Opening several files of different kinds at once loads each into its own tab; the tab for the first file is the one you see. If Dev Toolkit is already open, opening another file this way brings that same window to the front and adds to it, instead of starting a second copy of the app. This needs re-installing the app once after an update that adds it, and only Chrome and Edge on desktop support it — Safari, Firefox and mobile browsers don't, so there the file just opens as a download or in its usual app.
